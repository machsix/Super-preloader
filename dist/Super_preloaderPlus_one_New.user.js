  ...
  */
  ...
  (function () {
    ...
    loadSettings().then(function (values) {
      ...
      function init(window, document) {
          ...
          function insertedIntoDoc() {
            ...
            if (SSS.a_documentFilter) {
              try {
                ...
                logger.debug("[Super-preloader]", 'documentFilter executed successfully');
              } catch (e) {
            ...
            if (SSS.filter && typeof SSS.filter === 'function') {
              try {
                ...
                logger.debug("[Super-preloader]", 'Successfully executed filter(pages)');
              } catch (e) {
            ...
            if (paged >= SSS.a_maxpage) {
              logger.debug("[Super-preloader]", `Reached the maximum number of pages turned ${SSS.a_maxpage}`);
            ...
            if (nextlink && !pagedLinks.includes(nextlink)) {
              ...
              logger.error("[Super-preloader]", 'Next page link not found.', SSS.nextLink);
              removeL();
            }
          } // 返回,剩余高度是总高度的比值.
          ...
          autoPO = {
            startipages: function startipages(value) {
              if (value > 0) {
                ipagesmode = true;
                ipagesnumber = value + paged;
                notice('<b>Status</b>:' + 'Number of pages turned: <b>' + paged + '</b>,' + 'Continue turning to page <b style="color:red!important;">' + ipagesnumber + '</b>.');
        ...
        function prefetcher(SSS, floatWO) {
          ...
          if (SSS.useiframe) {
            ...
            got$1.get(nextlink, reqConf).then(function (res) {
              ...
              i = iarray.length;
              container.div2.innerHTML = 'Number of prefetched images: ' + '< b>' + i + '</b>' + '<br />' + 'Prefetch URL: ' + '< b>' + nextlink + '</b>';
              logger.info("[Super-preloader]", 'Number of prefetched images: ' + i + 'Prefetch URL: ' + nextlink);
        ...
        if (prefs.DisableI && window.self != window.parent) {
          ...
          if (isReturn) {
            logger.debug("[Super-preloader]", `The page with URL:${url} is a not a top-level window, JS execution is stopped.`);
            return;
          }
        }
        ...
        var findCurSiteInfo = function findCurSiteInfo() {
            ...
            if (Rurl.test(url)) {
              ...
              if (SII.autopager && SII.autopager.startFilter) {
                try {
                  ...
                  logger.debug("[Super-preloader]", 'startFilter execution successful');
                } catch (e) {
              ...
              if (!nextlink) {
                logger.warn("[Super-preloader]", 'Cannot find the next page link, continue searching for other rules, skiping rule:', SII);
                return "continue";
              ...
              if (!pageElement || Array.isArray(pageElement) && pageElement.length === 0) {
                nextlink = null;
                logger.error("[Super-preloader]", 'Unable to find the content, skiping rule:', SII, 'Continue searching for other rules');
                return "continue";
              }
          ...
          logger.debug("[Super-preloader]", `Total time spent on searching advanced rules and auto-matching:${new Date().getTime() - startTime.getTime()}ms`);
        };

        findCurSiteInfo(); // 上下页都没有找到啊

        if (!nextlink && !prelink) {
          logger.warn("[Super-preloader]", `No related links found, JS execution stopped. Total time:${new Date().getTime() - startTime.getTime()}ms`);
          return;
        } else {
        ...
        if (prefs.arrowKeyPage) {
          logger.debug("[Super-preloader]", 'Add left and right arrow keys to turn page listener.');
        ...
        logger.debug("[Super-preloader]", 'Add mouse gesture to autopager listener');
        ...
        if (!nextlink) {
          logger.error("[Super-preloader]", 'Next link does not exist, JS cannot continue.');
          logger.debug("[Super-preloader]", `Total processing time:${new Date().getTime() - startTime.getTime()}ms`);
          return;
        }
        ...
        if (!SSS.enable) {
          logger.warn("[Super-preloader]", 'This rule is disabled and script execution stops');
          logger.debug("[Super-preloader]", `Total processing time:${new Date().getTime() - startTime.getTime()}ms`);
          return;
        }

        logger.debug("[Super-preloader]", `Total processing time:${new Date().getTime() - startTime.getTime()}ms`); // 预读或者翻页.

        if (SSS.a_enable) {
          logger.debug("[Super-preloader]", 'Initialization, autopager mode.');
          autopager(SSS, floatWO);
        } else {
          logger.debug("[Super-preloader]", 'Initialization, preloader mode.');
        ...
        function autoGetLink(doc, win) {
          ...
          logger.debug("[Super-preloader]", `Number of links found:${i} Time spent:${new Date().getTime() - startTime.getTime()}ms`); //@ts-ignore
    ...
    function createDocumentByString(str) {
      // string转为DOM
      if (!str) {
        logger.error("[Super-preloader]", 'No strings were found to convert to DOM.');
        return;
      }
