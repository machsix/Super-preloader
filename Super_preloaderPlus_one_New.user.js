...
(function () {
	...
	*/
	...
	var SCRIPT_INFO = {
	  ...
	  version: pkg.version,
	  license: pkg.license,
	  changelog: "".concat(emoji('1F92A'), " Adguard is supported"),
	...
	SCRIPT_INFO.downloadURL = "".concat(SCRIPT_INFO.greasyfork, "/code/").concat(SCRIPT_INFO.name, ".user.js");
	SCRIPT_INFO.metaURL = "".concat(SCRIPT_INFO.greasyfork, "/code/").concat(SCRIPT_INFO.name, ".meta.js"); // New notification to send
	...
	function encodeURIE(text) {
	  ...
	  return [].map.call(text, function (x) {
	    if (isURIcomponent(x)) {
	      ...
	      return "%".concat(encodeHex(x).map(function (x) {
	...
	var RuleProvider = /*#__PURE__*/function () {
	  /**
	  ...
	  function RuleProvider(name, url, detailUrl) {
	    ...
	    classCallCheck(this, RuleProvider);
	  ...
	  createClass(RuleProvider, [{
	    ...
	    key: "downloadRule",
	    value: function () {
	      var _downloadRule = asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee() {
	          ...
	          while (1) {
	            switch (_context.prev = _context.next) {
	              case 0:
	                rule = [];
	                e = null;
	                _iterator = _createForOfIteratorHelper$1(this.url);
	                _context.prev = 3;
	              ...
	              case 16:
	                ...
	                logger.error("[Super-preloader]", "[DownloadRule] ".concat(this.name, " from ").concat(url, " [Status] ").concat(_context.t0));
	                e = _context.t0;
	        ...
	        }, _callee, this, [[3, 24, 27, 30], [7, 16]]);
	      }));

	      function downloadRule() {
	        return _downloadRule.apply(this, arguments);
	      }
	    ...
	    key: "updateRule",
	    value: function () {
	      var _updateRule = asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee2(lastUpdate) {
	          ...
	          while (1) {
	            switch (_context2.prev = _context2.next) {
	              ...
	              case 7:
	                ...
	                logger.error("[Super-preloader]", "[UpdateRule] ".concat(this.name, " [Status] ").concat(_context2.t0)); // mimic Promise.allSettled
	        ...
	        }, _callee2, this, [[1, 7], [14, 22]]);
	      }));

	      function updateRule(_x) {
	...
	var jsonRuleLoader = {
	  ...
	  updatePeriodInDay: 1,
	  resetExpire: function resetExpire() {
	        ...
	        while (1) {
	          switch (_context3.prev = _context3.next) {
	            ...
	            case 3:
	              ...
	              _context3.next = 6;
	              return GM.setValue('SITEINFO_json', _this.rule);
	        ...
	        while (1) {
	          switch (_context4.prev = _context4.next) {
	            case 0:
	              ...
	              lastUpdate = new Date(+_this2.expire - _this2.updatePeriodInDay * 24 * 60 * 60 * 1000);
	              today = new Date();

	              if (!(today > _this2.expire)) {
	                _context4.next = 10;
	                break;
	              }

	              promises = _this2.providers.map(function (x) {
	              ...
	              return Promise.all(promises).then(function (values) {
	                ...
	                if (status.every(function (x) {
	                  ...
	                  _this2.rule = values.map(function (_ref2) {
	                    var value = _ref2.value;
	                    return value ? value : _this2.rule;
	                  });
	                  _this2.expire = new Date(+today + _this2.updatePeriodInDay * 24 * 60 * 60 * 1000);
	                  logger.info("[Super-preloader]", "[UpdateRule] Next update at: ".concat(_this2.expire));
	        ...
	        while (1) {
	          switch (_context5.prev = _context5.next) {
	            ...
	            case 3:
	              ...
	              if (!(forceUpdateRule || _this3.getRule().length === 0)) {
	                _context5.next = 15;
	                break;
	              }

	              _context5.next = 15;
	              return _this3.updateRule(true);

	            case 15:
	              return _context5.abrupt("return", _this3.getRule());
	...
	function mergeProperty(oldProp, newProp) {
	  ...
	  for (var _i = 0, _Object$keys = Object.keys(oldProp); _i < _Object$keys.length; _i++) {
	    ...
	    if (!Array.isArray(oldProp[k]) && _typeof_1(oldProp[k]) === 'object') {
	...
	(function () {
	  ...
	  loadSettings().then(function (values) {
	    ...
	    function init(window, document) {
	      ...
	      logger.debug("[Super-preloader]", "Page url is: ".concat(url, ", JS loaded successfully")); // 第一阶段..分析高级模式..
	      ...
	      logger.debug("[Super-preloader]", "Total time spent:".concat(new Date().getTime() - startTime.getTime(), "ms")); // 预读或者翻页.
	      ...
	      function autoGetLink(doc, win) {
	        ...
	        logger.debug("[Super-preloader]", "Time to search ".concat(i, " links:").concat(new Date().getTime() - startTime.getTime(), "ms")); //@ts-ignore
